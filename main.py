...
# from agents.emotion_chat_agent import EmotionChatAgent
# from models.schemas import ChatRequest, ChatResponse, SessionInfo
...
# # Initialize FastAPI app
# app = FastAPI(
#     title="EmotionRead API",
#     description="AI-powered book recommendation chatbot based on emotional analysis using Google Gemini",
#     version="1.0.0"
# )
...
# # Load book data from CSV
# DATA_PATH = "./books_with_emotions.csv"
...
#         session_id = str(uuid.uuid4())
#         emotion_sessions[session_id] = {
#             "agent": EmotionChatAgent(DATA_PATH),
#             "created_at": datetime.now(),
#             "messages": [],
#             "processing": False,
...
#         emotion_sessions[session_id]["last_activity"] = datetime.now()
#         return emotion_sessions[session_id]
...
# # WebSocket endpoint for real-time chat with emotion detection
# @app.websocket("/emotion/ws/{session_id}")
# async def emotion_websocket_endpoint(websocket: WebSocket, session_id: str):
#     try:
#         # Verify session exists
...
#             "type": "connection",
#             "status": "connected",
#             "message": "Connected to EmotionRead! Let's chat about books that match your mood."
#         })
        
#         # Send first message to start conversation
#         first_message = await session["agent"].get_response("")
#         timestamp = datetime.now().isoformat()
...
#                 response = await session["agent"].get_response(user_message)
...
#     return SessionInfo(
#         session_id=session_id, 
#         message="Emotion session created successfully",
...
#     try:
#         # Process message and generate response
#         response = await session["agent"].get_response(request.message)
...
# Import BookPal dependencies
from agents.chatbot_agent import BookChatAgent
# Import EmotionRead dependencies - using the new implementation
from agents.emotion_chat_agent import EmotionChatAgent
from models.schemas import ChatRequest, ChatResponse, DocumentUploadResponse, SessionInfo, EmotionProfile, BookRecommendation
...
#################################################
# EmotionRead App - Updated with Gemini Integration
#################################################
...
class EmotionSessionManager:
    ...
    def create_session() -> str:
        session_id = str(uuid.uuid4())
        emotion_sessions[session_id] = {
            "agent": EmotionChatAgent(DATA_PATH),  # Using the new agent implementation
            "created_at": datetime.now(),
            "messages": [],
            "processing": False,
            "last_activity": datetime.now()
...
async def emotion_chat(session_id: str, request: ChatRequest):
    ...
    try:
        # Process message and generate response directly (new agent handles async)
        response = await session["agent"].get_response(request.message)
