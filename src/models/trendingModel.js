  ...
  return data.items
    ...
    .map(item => {
      ...
      return {
        ...
        thumbnail: buildImageUrl(info.imageLinks?.thumbnail, isbn10, isbn13),
...
// Save books to both models with uniqueness check
async function saveBooksToModels(books) {
  ...
  for (const book of books) {
    try {
      ...
      try {
        await TrendingBook.create(book);
        results.trending.added++;
        console.log(`✅ Trending: "${book.title}"`);
      } catch (trendingError) {
